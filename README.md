...
### 🔸평가 방식
- 총 9개 질문(일상 주행, 돌발 상황, 법규/상식), 6개의 항목, 5점 만점 기준으로 평가함
...
- **사용자 질문 라우팅 시도** :
- **대화 맥락 유지** : 메모리 기능을 활용해 사용자와의 이전 대화를 기억해 맥락에 맞는 답변 제공 
...
|**박민정**| 신뢰성이 중요한 RAG 데이터셋을 찾는 과정이 매우 힘들었지만 정제하고 아키텍처를 구축해 나가는 과정이 뿌듯했습니다. |
