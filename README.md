...
<img width="1355" height="732" alt="image" src="https://github.com/user-attachments/assets/dba11912-4e9c-415d-95bd-9984a925fcf5" />
...
1. **✅파라미터 확대**
    ...
    - 3B 모델은 1.5B 모델 대비 긴 문맥 이해와 조건문 해석의 강점을 갖고 있으며, 여러 법령 조항을 동시에 참조해야하는 경우 안정적인 추론이 가능함
...
#### 1. 프롬프트 엔지니어링
- 고품질의 답변을 위해 프롬프트를 길고 명확하게 작성할수록 오히려 답변의 품질이 떨어지는 현상
...
#### 3. 메모리 기능 추가 
- 후속 질문이나 대명사를 이해하지 못해 대화 단절 문제
- 이전 대화도 기억해내기 위해 메모리 기능 추가 시도

#### 4. 하이퍼 서치
-  내용 기반 임베딩만으로는 유사도 검색 정확도가 떨어져, 제목과 내용을 별도로 임베딩하고 가중치를 부여하는 하이퍼 서치를 도입하여 검색 성능을 높임

#### 5. 파인 튜닝
- 법령 질문은 평이한 일상어(예: “편의점 앞에 차 대도 돼?”)로 들어오는 경우가 많음.
- 이러한 질문만으로는 유사도 기반 검색에서 법령 문구를 정확히 찾기 어려워, 법령 키워드를 추출하는 과정을 학습시켜 검색 성능을 향상시킴.
---
...
<table>
  <tr>
    <th width="10%">임길진</th>
    <td>법령이라는 특수한 데이터셋을 전처리해 벡터 DB에 삽입하는 과정에서 내용만 임베딩했을 때는 유사도 검색이 잘 되지 않아 시행착오를 겪었지만, 법령의 제목과 내용을 분리해 임베딩하고 하이퍼 서치를 적용했으며, 평문에서 법령 키워드를 추출하는 파인튜닝을 통해 3B 규모 모델에서도 만족스러운 성능의 법령 QA를 구현할 수 있었던 점이 뿌듯했습니다.</td>
  </tr>
  <tr>
    <th>박민정</th>
    <td>신뢰성이 중요한 RAG 데이터셋을 수집하고 정제하는 과정이 정말 오래 걸리고 힘들었지만 이 데이터셋이 결국 답변의 퀄리티를 좌우한다는 걸 알기에 끝까지 열심히 했던 것 같습니다. 그래도 RAG를 붙였음에도 답변이 생각만큼 잘 나오지 않아서 답답했고 시간이 부족해서 파인튜닝을 못 한 게 많이 아쉬웠습니다. 다음 프로젝트에서는 이번에 계획만 하고 실천하지 못했던 STT, TTS 기술과 파인튜닝을 적용해서 완성도를 높여보고 싶습니다.</td>
  </tr>
  <tr>
    <th>이가은</th>
