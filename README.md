...
| **전처리 내용** | 1. 구조 단위 저장 <br> - 기본 단위: 조–항–호 <br> - 단, 호 단위만 단독으로 저장하면 문맥이 끊기므로, 각 항 단위에 포함된 모든 호를 묶어 "항내용 + 호내용…" 형태로 병합 저장.<br>2. 불필요 조항 제거 (패턴 필터링) <br> - 특정 키워드가 포함된 문구(예: “제\d+항에 따른”, “제\d+조에 따른” 등)만 포함한 조항은 실질적 내용이 없으므로 제거.<br>3. 주제에 벗어난 내용 제거 <br> - 교통 어시스턴트 목적과 직접 관련이 없는 조항을 제외.<br> - 제거 대상 키워드 예시:<br> &nbsp;&nbsp;- 정의 (법률 전체 개념 정의) <br> &nbsp;&nbsp;- 학원, 수강 (운전면허 학원 외 일반 교육/학원 관련 규정)<br> &nbsp;&nbsp;- 교통안전과 직접 관련 없는 행정 절차, 기관 운영, 개인정보 처리 등|
| **최종 형식** | JSON {"id":"...법_제n조...", "content":"...", "metadata": {"source_type": "law", "article_title":"...", ...}} |
